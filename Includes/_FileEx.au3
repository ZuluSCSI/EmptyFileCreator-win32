...
Func _FileEx_SetFileTime($sPath, ByRef $tCreated, ByRef $tAccessed, ByRef $tModified)
    ...
    If IsDllStruct($tCreated) Then $pC = DllStructGetPtr($tCreated)
    If IsDllStruct($tAccessed) Then $pA = DllStructGetPtr($tAccessed)
    If IsDllStruct($tModified) Then $pM = DllStructGetPtr($tModified)
    ...
    Local $err = 0
    Local $ret = DllCall("kernel32.dll", "bool", "SetFileTime", "handle", $hFile, "ptr", $pC, "ptr", $pA, "ptr", $pM)
    If @error Or (Not $ret[0]) Then $err = 1
    _WinAPI_CloseHandle($hFile)
    Return SetError($err, 0, Abs($err - 1))
